...
test <- cor.test(datos26$Height, datos26$Wr.Hnd, method = "pearson", use = "complete.obs")
cat("Coeficiente de correlación de Pearson: r =", round(test$estimate, 3), "\n")
cat("p-valor =", round(test$p.value, 4), "\n")

if (test$p.value < 0.05) {
  cat("Existe una relación lineal significativa.\n")
} else {
  cat("No hay evidencia de relación lineal significativa.\n")
}
...
med_h  <- median(datos26$Height, na.rm = TRUE)
mad_h  <- mad(datos26$Height,    na.rm = TRUE)
limite <- 3 * mad_h

cat("Mediana de Height:", med_h, "\n")
cat("MAD de Height:", round(mad_h, 3), "\n")
cat("Umbral Hampel (+-3·MAD):", round(limite, 3), "\n\n")

outliers_h <- datos26 %>%
  filter(!is.na(Height) & abs(Height - med_h) > limite)
